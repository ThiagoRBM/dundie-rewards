from subprocess import (  # com isso é possível rodar qualquer comando
    CalledProcessError,
    check_output,
)
...
def test_negative_call_load_command_with_wrong_parameters(wrong_command):
    ...
    with pytest.raises(
        CalledProcessError
    ) as error:  # erro de digitação ("loady") gera o CalledProcessError
        check_output(
            [
                "dundie",
                wrong_command,
                "tests/assets/people.csv",
            ]  # recebe a lista do decorator
        ).decode("utf-8").split(
            "\n"
        )  # comandos que são passados no terminal
